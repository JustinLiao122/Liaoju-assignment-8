...
def do_experiments(start, end, step_num):
    ...
    for i, distance in enumerate(shift_distances, 1):
        ...
        plt.plot(np.linspace(X[:, 0].min(), X[:, 0].max(), 200),
                 slope * np.linspace(X[:, 0].min(), X[:, 0].max(), 200) + intercept, color='black')
        ...
        logistic_loss = -np.mean(np.log(probabilities[range(len(y)), y.astype(int)]))
    ...
    plt.subplot(3, 3, 5)
    plt.plot(shift_distances, intercept_list, marker='o', label="Intercept Ratio")
